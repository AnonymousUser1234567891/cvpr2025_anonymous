...
json_path = "configs/Labels.json"
# json_path = "configs/Labels2_1k.json"
...
# initialize when class is define.
lbl_text = None
...
class ImageNetDataset(Dataset):
    def __init__(self, cfg):
        ...
        print(f"Total classes: {len(self.label)}")
        self.file = [
            os.path.join(root, i.strip()) 
                for i in open(cfg["file"], 'r').readlines()
                    if i.strip().split(os.sep)[-2] in self.label
        ]
...
class PretrainImageNetDataset(ImageNetDataset):
    def __init__(self, cfg):
        ...
        self.img_augmentation_view1, self.img_augmentation_view2 = get_augmentation(
            cfg, pop_resize = self.point_level_aug)
        self.loader = partial(
            self.loader,
            augment=base_augment(self.mode), 
            neglect_polarity=self.cfg.get('neglect_polarity', False),
            global_time=self.cfg.get('global_time', True), 
            strict=self.cfg.get('strict', False), 
            use_image=self.cfg.get('use_image', True),
            denoise_sort=self.cfg.get('denoise_sort', False), 
            denoise_image=self.cfg.get('denoise_image', False),
            filter_flash=self.cfg.get('filter_flash', False), 
            filter_noise=self.cfg.get('filter_noise', False),
            quantize_sort=self.cfg.get('quantize_sort', None)
        )
    ...
    def get_image(self, path):
        if 'tmp' in path:
            path = path.replace("extracted_train", "train")
            path = path.replace("extracted_val", "val")
            path = path.replace("npz", "JPEG")
            path = path.replace("tmp", "ImageNet")
        else:
            path = path.replace("extracted_train", "train")
            path = path.replace("extracted_val", "val")
            path = path.replace("npz", "JPEG")
            path = path.replace("N_ImageNet", "ImageNet")
        ...
        img = transform.Resize((224,224))(img)
        return img

    def __getitem__(self, idx):
        event_path = self.file[idx]
        label = self.get_label(event_path)
        text = os.path.basename(os.path.dirname(event_path))

        if "emb_path" in self.cfg:
            emb = self.get_emb(event_path)
            img = self.get_image(event_path)
        else:
            emb = torch.ones(1)
        ...
        # event2 = self.img_augmentation_view2(event2) if self.mode == 'train' else event2
