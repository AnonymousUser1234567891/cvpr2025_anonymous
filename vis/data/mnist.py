...
text_inputs = torch.cat(
    [
        clip.tokenize(f"a photo of number {label}") 
...
class NMINIST(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # plt.imshow(events_data.transpose(1, 2, 0))
        # plt.savefig("events_data.png")
        # plt.clf()
        # import pdb; pdb.set_trace()
        events_data = events_data.mean(axis=1)
        # plt.imshow(events_data.shape)
        # plt.savefig("events_data.png")
        # plt.close()
        data = {
            "img": img,
